...
MODULE idaho_chiral_potential
  ...
  TYPE(chp_int_type) , PRIVATE :: chp_use_2PE_1loop_r ! 1 or 0
  ...
  LOGICAL, PRIVATE :: chp_use_error_term
  TYPE(chp_real_type_RA), PRIVATE :: chp_error_term(1:5)
  TYPE(chp_real_type), PRIVATE :: chp_error_term_n(1:5)
  TYPE(chp_real_type), PRIVATE :: chp_error_term_q2_exp(1:5)
  TYPE(chp_real_type), PRIVATE :: chp_error_term_factor(1:5)
 ...
 CONTAINS
  ...
  SUBROUTINE chp(pout, pin, coup, S, j, T, tz, POT)
    ...
    REAL(8)            :: v_error_term(6), v_error_term_tmp(6)
    ...
    ELSE
       ...
       IF (chp_chiral_order%val > NLO) THEN
          ...
          if(chp_use_2PE_1loop_r%val == 1) then
       ...
       IF (chp_chiral_order%val > NNLO)THEN
          ...
          IF (chp_ren%name == 'DR') THEN
             ...
             Wc = Wc + iso(T) * (chp_N3LO_2PE_Wc_2loop_DR_a(    q2, loop_w, loop_L, loop_wtilde2,         2) + &
                                 chp_N3LO_2PE_Wc_2loop_DR_b(    q2,                                       2))
    ...
    IF (chp_use_error_term) THEN
        if(chp_error_term(1)%set) then
            VT = chp_error_term_T(q2)
            v_error_term_tmp = 0
            CALL pwd_T(VT, CHN, pfinal, pinit, v_error_term_tmp)
            v_error_term = v_error_term + v_error_term_tmp * freg(pfinal, pinit, chp_error_term_n(1)%val)
        end if
        if(chp_error_term(2)%set) then
            Vc = chp_error_term_c(q2)
            v_error_term_tmp = 0
            CALL pwd_c(Vc, CHN, pfinal, pinit, v_error_term_tmp)
            v_error_term = v_error_term + v_error_term_tmp * freg(pfinal, pinit, chp_error_term_n(2)%val)
        end if
        if(chp_error_term(3)%set) then
            Vs = chp_error_term_s(q2)
            v_error_term_tmp = 0
            CALL pwd_s(Vs, CHN, pfinal, pinit, v_error_term_tmp)
            v_error_term = v_error_term + v_error_term_tmp * freg(pfinal, pinit, chp_error_term_n(3)%val)
        end if
        if(chp_error_term(4)%set) then
            VLS = chp_error_term_LS(q2)
            v_error_term_tmp = 0
            CALL pwd_LS(VLS, CHN, pfinal, pinit, v_error_term_tmp)
            v_error_term = v_error_term + v_error_term_tmp * freg(pfinal, pinit, chp_error_term_n(4)%val)
        end if
        if(chp_error_term(5)%set) then
            VsigL = chp_error_term_sigL(q2)
            v_error_term_tmp = 0
            CALL pwd_sigL(VsigL, CHN, pfinal, pinit, v_error_term_tmp)
            v_error_term = v_error_term + v_error_term_tmp * freg(pfinal, pinit, chp_error_term_n(5)%val)
        end if
        v_error_term = v_error_term * const(1)*chp_mnuc(CHN%tz)%val/DSQRT(cm_energy(pinit,CHN%tz)*cm_energy(pfinal,CHN%tz))
    END IF
  ...
  FUNCTION chp_NNLO_two_pion_exchange_Vc(q2, w, A, wt2, impi, imnuc) RESULT(res)
    ...
    res = 0.0D0
    if(chp_use_2PE_1loop_r%val == 0) then
      res = const(9)*(- &
...
!$OMP parallel do schedule(static) default(none) shared(q2, impi, res, fact, a1, chp_2PE_2loop_int_mesh_z_VC_SFR, z_nr, gA2, m_pi_inv, d1, d2, d3, d4, d5, L) private(i, j, s, s2, z, z2, zt, x)
...
!$OMP parallel do schedule(static) default(none) shared(q2, impi, res, fact, a1, chp_2PE_2loop_int_mesh_z, chp_2PE_2loop_int_WC_DR_x_fact) private(i)
    DO i = 1, nof_theta_int_points
      res(i) = fact * q2(i)**3 * sum(chp_2PE_2loop_int_mesh_z%pnt(:)%w * chp_2PE_2loop_int_WC_DR_x_fact(:) / (a1 + chp_2PE_2loop_int_mesh_z%pnt(:)%x**2 * q2(i)))
    END DO
!$OMP end parallel do
...
!$OMP end parallel do
  ...
  FUNCTION chp_error_term_T(q2) RESULT(res)
    REAL(8), INTENT(IN) :: q2(1:nof_theta_int_points)
    REAL(8) :: res(1:nof_theta_int_points)

    res = chp_error_term(1)%val * chp_error_term_factor(1)%val * q2**chp_error_term_q2_exp(1)%val
  END FUNCTION chp_error_term_T

  FUNCTION chp_error_term_c(q2) RESULT(res)
    REAL(8), INTENT(IN) :: q2(1:nof_theta_int_points)
    REAL(8) :: res(1:nof_theta_int_points)

    res = chp_error_term(2)%val * chp_error_term_factor(2)%val * q2**chp_error_term_q2_exp(2)%val
  END FUNCTION chp_error_term_c

  FUNCTION chp_error_term_s(q2) RESULT(res)
    REAL(8), INTENT(IN) :: q2(1:nof_theta_int_points)
    REAL(8) :: res(1:nof_theta_int_points)

    res = chp_error_term(3)%val * chp_error_term_factor(3)%val * q2**chp_error_term_q2_exp(3)%val
  END FUNCTION chp_error_term_s

  FUNCTION chp_error_term_LS(q2) RESULT(res)
    REAL(8), INTENT(IN) :: q2(1:nof_theta_int_points)
    REAL(8) :: res(1:nof_theta_int_points)

    res = chp_error_term(4)%val * chp_error_term_factor(4)%val * q2**chp_error_term_q2_exp(4)%val
  END FUNCTION chp_error_term_LS

  FUNCTION chp_error_term_sigL(q2) RESULT(res)
    REAL(8), INTENT(IN) :: q2(1:nof_theta_int_points)
    REAL(8) :: res(1:nof_theta_int_points)

    res = chp_error_term(5)%val * chp_error_term_factor(5)%val * q2**chp_error_term_q2_exp(5)%val
  END FUNCTION chp_error_term_sigL
  ...
  SUBROUTINE initialize_chiral_potential
    ...
    chp_use_2PE_2loop_int = chp_int_type(1, '2PE 2loop int', .TRUE.)
    chp_use_2PE_1loop_r = chp_int_type(1, '2PE 1loop r', .TRUE.)
    chp_2PE_2loop_int_VTS_data_set = .FALSE.
    chp_2PE_2loop_int_WC_DR_data_set  = .FALSE.
    chp_2PE_2loop_int_WC_SFR_data_set  = .FALSE.
    ...
    chp_use_error_term = .false.
    chp_error_term(1) = chp_real_type_RA(RAirrel, 'err-T', .FALSE.)
    chp_error_term(2) = chp_real_type_RA(RAirrel, 'err-C', .FALSE.)
    chp_error_term(3) = chp_real_type_RA(RAirrel, 'err-S', .FALSE.)
    chp_error_term(4) = chp_real_type_RA(RAirrel, 'err-LS', .FALSE.)
    chp_error_term(5) = chp_real_type_RA(RAirrel, 'err-sigL', .FALSE.)
    chp_error_term_n(1) = chp_real_type(rirrel, 'err-T-n', .FALSE.)
    chp_error_term_n(2) = chp_real_type(rirrel, 'err-C-n', .FALSE.)
    chp_error_term_n(3) = chp_real_type(rirrel, 'err-S-n', .FALSE.)
    chp_error_term_n(4) = chp_real_type(rirrel, 'err-LS-n', .FALSE.)
    chp_error_term_n(5) = chp_real_type(rirrel, 'err-sigL-n', .FALSE.)
    chp_error_term_q2_exp(1) = chp_real_type(rirrel, 'err-T-q2', .FALSE.)
    chp_error_term_q2_exp(2) = chp_real_type(rirrel, 'err-C-q2', .FALSE.)
    chp_error_term_q2_exp(3) = chp_real_type(rirrel, 'err-S-q2', .FALSE.)
    chp_error_term_q2_exp(4) = chp_real_type(rirrel, 'err-LS-q2', .FALSE.)
    chp_error_term_q2_exp(5) = chp_real_type(rirrel, 'err-sigL-q2', .FALSE.)
    chp_error_term_factor(1) = chp_real_type(rirrel, 'err-T-fac', .FALSE.)
    chp_error_term_factor(2) = chp_real_type(rirrel, 'err-C-fac', .FALSE.)
    chp_error_term_factor(3) = chp_real_type(rirrel, 'err-S-fac', .FALSE.)
    chp_error_term_factor(4) = chp_real_type(rirrel, 'err-LS-fac', .FALSE.)
    chp_error_term_factor(5) = chp_real_type(rirrel, 'err-sigL-fac', .FALSE.)
  ...
  SUBROUTINE chp_set_units_and_derive_constants
    ...
    END IF
    DO i = 1, 5
       IF(chp_error_term(i)%set) THEN
           IF(.NOT. chp_error_term_q2_exp(i)%set) error stop 'must set error_term_q2_exp'
           IF(.NOT. chp_error_term_factor(i)%set) error stop 'must set error_term_factor'
       END IF
       IF(chp_error_term_n(i)%set) CYCLE
       CALL chp_set_error_term_reg_par(i, 2.0D0)
    END DO
  ...
  SUBROUTINE chp_print_constants(title,unit)
    ...
    IF (chp_use_error_term) THEN
       DO i=1,5
          if(.not. chp_error_term(i)%set) cycle
          CALL chp_print_rconst_if(unit, chp_error_term(i))
       END DO
    END IF
...
END SUBROUTINE chp_print_cconst
  ...
  SUBROUTINE chp_set_error_term_reg_par(pwd_idx, val)
    INTEGER, INTENT(IN) :: pwd_idx
    REAL(8), INTENT(IN) :: val

    IF (.NOT. chp_chiral_order%set) THEN
       STOP 'Must set chiral order before setting regulator parameters'
    END IF
    chp_error_term_n(pwd_idx)%set = .TRUE.
    chp_error_term_n(pwd_idx)%val = val
  END SUBROUTINE chp_set_error_term_reg_par

  SUBROUTINE chp_set_use_error_term(val)

    LOGICAL, INTENT(IN) :: val

    chp_use_error_term = val
  END SUBROUTINE chp_set_use_error_term
   
  SUBROUTINE chp_set_error_term(pwd_idx, val)    
    
    INTEGER, INTENT(IN) :: pwd_idx
    REAL(8), INTENT(IN) :: val

    chp_error_term(pwd_idx)%val  = val
    chp_error_term(pwd_idx)%set  = .TRUE.

  END SUBROUTINE chp_set_error_term

  SUBROUTINE chp_set_error_term_q2_exp(pwd_idx, val)    
    
    INTEGER, INTENT(IN) :: pwd_idx
    REAL(8), INTENT(IN) :: val

    chp_error_term_q2_exp(pwd_idx)%val  = val
    chp_error_term_q2_exp(pwd_idx)%set  = .TRUE.

  END SUBROUTINE chp_set_error_term_q2_exp

  SUBROUTINE chp_set_error_term_factor(pwd_idx, val)    
    
    INTEGER, INTENT(IN) :: pwd_idx
    REAL(8), INTENT(IN) :: val

    chp_error_term_factor(pwd_idx)%val  = val
    chp_error_term_factor(pwd_idx)%set  = .TRUE.

  END SUBROUTINE chp_set_error_term_factor

  SUBROUTINE chp_get_error_term(pwd_idx, val, name)

    INTEGER, INTENT(IN) :: pwd_idx
    REAL(8), INTENT(INOUT) :: val
    CHARACTER(LEN=12), INTENT(INOUT) :: name

    IF (.NOT. chp_error_term(pwd_idx)%set) THEN
        WRITE(CHP_ERR, "(A,I5,A)") 'chp_error_term ', pwd_idx, ' NOT SET'
        STOP
    END IF

    val = chp_error_term(pwd_idx)%val
    name = chp_error_term(pwd_idx)%name

  END SUBROUTINE chp_get_error_term
  ...
  SUBROUTINE chp_set_2PE_1loop_r(set)
    
    INTEGER, INTENT(IN) :: set

    chp_use_2PE_1loop_r%val  = set
    chp_use_2PE_1loop_r%set  = .TRUE.
    
  END SUBROUTINE chp_set_2PE_1loop_r

  SUBROUTINE chp_get_2PE_1loop_r(get)
    
    INTEGER, INTENT(INOUT) :: get

    IF (.NOT. chp_use_2PE_1loop_r%set) THEN
       WRITE(CHP_ERR,"(A)") '2PE_1loop_r NOT SET'
       STOP
    END IF
    
    get = chp_use_2PE_1loop_r%val
    
  END SUBROUTINE chp_get_2PE_1loop_r
