...
class Gaussfit:
    ...
    def plot_modelerror(self, Xvalid, Xlearn, Yvalid):
        ...
        plt.axis([ -1.1*min(mindists), 1.1*max(mindists),  1.1*min(Ymodel-Yvalid), 1.1*max(Ymodel-Yvalid)])
        #TODO: decide between fig1 and fig2
        plt.figure(2)
        plt.plot(mindists, (Ymodel-Yvalid)/Yvalid, '.')
        plt.xlabel('Distance to closest training point')
        plt.ylabel('Vallidated relative error')
        plt.axis([1.1*min(mindists), 1.1*max(mindists), 1.1*min((Ymodel-Yvalid)/Yvalid), 1.1*max((Ymodel-Yvalid)/Yvalid)])
        #TODO: fix x-scale
        plt.show()
